...
**What (definition):**
- Solo vs group based on `V4234` (NCVS-2 CIR Question 60: "Was the crime committed by only one or by more than one offender?").
- V4234 coding: 1 = Only one, 2 = More than one, 3 = Don't know, 8 = Residue, 9 = Out of universe.
- Source: `docs/bjs/ncvs22_cir.pdf` (NCVS-2 Crime Incident Report, Q60 ONEORMOREOFFENDERS).

**Code:** `originaldata-pull/ncvs-data-coding.Rmd` and `build_merged_2014_2024.py`
- Derived: `solo_group_crime = case_when(V4234==1 ~ 'solo', V4234==2 ~ 'group', else NA)`
- Note: V4234==3 is "Don't know" (NOT "three offenders") and is correctly treated as NA.
