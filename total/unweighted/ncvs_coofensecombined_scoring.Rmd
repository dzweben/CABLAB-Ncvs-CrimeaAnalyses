...
```{r}
ncvs_df <- read.csv("/Users/dannyzweben/Desktop/CABLAB_Files/ncvs/ncvs-merged-data-2014-2022.csv")  # read CSV into dataframe
...
```{r}
# Normalize age labels to match expected levels
...
age_levels <- c("under_12", "12-14", "15-17", "18-20", "21-29", "30+")
 ...
 #Filter rows with valid age groups
valid_rows <- which(
  ncvs_df$oldest_multiple %in% age_levels &
  ncvs_df$youngest_multiple %in% age_levels
)

oldest <- factor(ncvs_df$oldest_multiple[valid_rows], levels = age_levels, ordered = TRUE)
youngest <- factor(ncvs_df$youngest_multiple[valid_rows], levels = age_levels, ordered = TRUE)
...
names(gap_table) <- paste(names(gap_table), "groups apart")

 #Summary values
total_with_gap <- sum(age_diff >= 2)
total_valid_rows <- length(valid_rows)
prop_with_gap <- round(total_with_gap / total_valid_rows, 4)

 #Output APA-style table
summary_df <- data.frame(
  `Gap Size` = names(gap_table),
  Frequency = as.integer(gap_table)
)

summary_df <- rbind(
  summary_df,
  data.frame(`Gap Size` = "Total with Age Gap", Frequency = total_with_gap),
  data.frame(`Gap Size` = "Proportion of Total", Frequency = prop_with_gap)
)
...
# --- Define age_levels (you already did this) ---
# age_levels <- c("under_12", "12-14", "15-17", "18-20", "21-29", "30+")

# --- Filter valid co-offense rows ---
valid_rows <- which(
  ncvs_df$oldest_multiple %in% age_levels &
  ncvs_df$youngest_multiple %in% age_levels
)

# --- Build symmetrical co-offense pairings ---
pair_list <- list()
for (i in valid_rows) {
...
# --- Frequency table (offender × co-offender) ---
pair_counts <- table(
  factor(pair_df$offender, levels = age_levels),
  factor(pair_df$co_with, levels = age_levels)
)

# --- Create row-normalized % matrix, handling 0s safely ---
pair_percents <- matrix(NA, nrow = nrow(pair_counts), ncol = ncol(pair_counts))
rownames(pair_percents) <- rownames(pair_counts)
colnames(pair_percents) <- colnames(pair_counts)

nonzero_rows <- rowSums(pair_counts) > 0
pair_percents[nonzero_rows, ] <- round(prop.table(pair_counts[nonzero_rows, , drop = FALSE], margin = 1) * 100, 1)

# --- Add % labels and replace NaN/NA with em dashes ---
pair_percent_labels <- apply(pair_percents, c(1, 2), function(x) {
  if (is.na(x) || is.nan(x)) "—" else paste0(x, "%")
})

# --- Convert to labeled data frame ---
...
# --- Reorder columns to match age_levels ---
ncvs_pairing_df <- ncvs_pairing_df[, c("Offender Age", age_levels)]


# --- Coerce all cells to character (except row label) ---
ncvs_pairing_df <- ncvs_pairing_df |>
  mutate(across(-`Offender Age`, as.character))

# --- Format APA-style flextable ---
...
write.csv(ncvs_pairing_df, file = "/Users/dannyzweben/Desktop/CABLAB_Files/ncvs/total/R_Tables/cooffense_pairing_matrix.csv", row.names = FALSE)
...
observed_rows <- with(ncvs_df, social_crime == "observed")
...
# Save to Word document (fixing path string)
save_as_docx(
  "Table X. Distribution of Crime Involvement by Age Group" = ft,
  path = "/Users/dannyzweben/Desktop/CABLAB_Files/ncvs/total/R_Tables/combinedco_ncvs_calc_1_table.docx"
)
...
ncvs_calc_1_percentages[, c("alone", "group", "observed")] <- round(
  ncvs_calc_1_no_total[, c("alone", "group", "observed")] / ncvs_calc_1_no_total$grand_total * 100, 1
)
...
save_as_docx(
  ...
  path = "/Users/dannyzweben/Desktop/CABLAB_Files/ncvs/total/R_Tables/combinedco-ncvs_percentages_table.docx"
)
...
save_as_docx(
  ...
  path = "/Users/dannyzweben/Desktop/CABLAB_Files/ncvs/total/R_Tables/combinedco-chi_squared_table.docx"
)
...
save_as_docx(
  ...
  path = "/Users/dannyzweben/Desktop/CABLAB_Files/ncvs/total/R_Tables/Combinedcof-15-17int-logistic_regression_solo_offending.docx"
)
```
...
print(doc, target = "/Users/dannyzweben/Desktop/CABLAB_Files/ncvs/total/R_Tables/Combinedcof-12-14int-logistic_regression_solo_offending.docx")
...
save_as_docx(
  ...
  path = "/Users/dannyzweben/Desktop/CABLAB_Files/ncvs/total/R_Tables/Combinedcof-18-20-int-logistic_regression_solo_offending.docx"
)
